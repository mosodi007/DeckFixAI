...
export function analyzeSlideComplexity(slideContent: SlideContent): ComplexityResult {
...
export function estimateMultipleSlidesCost(slides: SlideContent[]): {
  totalCost: number;
  averageCost: number;
  breakdown: ComplexityResult[];
} {
  const breakdown = slides.map(slide => analyzeSlideComplexity(slide));
  const totalCost = breakdown.reduce((sum, result) => sum + result.creditCost, 0);
  const averageCost = Math.round(totalCost / slides.length);
