  ...
  try {
    ...
    try {
      ...
      if (confirmationError) {
        console.error('Error generating verification link:', JSON.stringify(confirmationError, null, 2));
        throw confirmationError;
      }
      
      if (!confirmationData?.properties?.action_link) {
        console.error('No action_link in response:', JSON.stringify(confirmationData, null, 2));
      ...
      console.log('Successfully generated secure verification link with token and expiry');
    } catch (error: any) {
      console.error('Failed to generate admin verification link. Error details:', {
      ...
      // Return error response instead of using insecure fallback
      return new Response(
        JSON.stringify({ 
          error: 'Failed to generate secure verification link',
